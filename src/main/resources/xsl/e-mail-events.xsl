...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mcrmods="xalan://org.mycore.mods.MCRMODSClassificationSupport"
  ...
  <xsl:param name="MCR.clausthal-module.EditorMail" />
  <xsl:variable name="newline" select="'&#xA;'" />
  <xsl:variable name="categories" select="document('classification:metadata:1:children:clausthal_institutes')/mycoreclass/categories" />
  <xsl:variable name="institutemember" select="$categories/category[mcrxsl:isCurrentUserInRole(concat('clausthal_institutes:',@ID))]" />
  ...
  <xsl:template match="/">
    ...
    <email>
      <from><xsl:value-of select="$MCR.clausthal-module.EditorMail" /></from>
  ...
  <xsl:template match="mycoreobject" mode="email">
    <xsl:choose>
      <xsl:when test="not(mcrxsl:isCurrentUserInRole('editor') or mcrxsl:isCurrentUserInRole('admin')) and ($action='create')">
        <!-- SEND EMAIL -->
        ...
        <body>
          <xsl:value-of select="concat('Ein neues Objekt(',$objectType,') wurde erstellt:')" />
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          ...
          <xsl:when test="not(mcrxsl:isCurrentUserInRole('submitter'))">
            <xsl:message>
              <xsl:value-of select="concat('Do not send mail as current user ',$CurrentUser, ' is not in group submitter.')" />
            </xsl:message>
          </xsl:when>
          <xsl:otherwise>
            <xsl:message>
  ...
  <xsl:template match="mycoreobject" mode="mailReceiver">
    <xsl:if test="string-length($MCR.clausthal-module.EditorMail)&gt;0">
      <to>
        <xsl:value-of select="$MCR.clausthal-module.EditorMail" />
      </to>
    </xsl:if>
    <!-- xsl:for-each select="$institutemember">
      <xsl:choose>
        <xsl:when test="./@ID='xx'">
          <to>xx@tu-clausthal.de</to>
        </xsl:when>
      </xsl:choose>
    </xsl:for-each -->
  </xsl:template>
