...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mods="http://www.loc.gov/mods/v3"
  ...
  <xsl:import href="xslImport:solr-document:clausthal-solr.xsl" />

  <xsl:template match="mycoreobject[contains(@ID,'_mods_')]">
    <xsl:variable name="status" select="mcrxsl:isInCategory(@ID,'clausthal_status:published')" />
    <xsl:apply-imports />
    <!-- fields from mycore-mods -->
    <xsl:apply-templates select="metadata/def.modsContainer/modsContainer/mods:mods" mode="clausthal">
 ...
 <xsl:template match="mods:mods" mode="clausthal">
...
<!--         </xsl:message> -->
    ...
    <xsl:if test="./mods:genre[@type='kindof']">
      <field name="mods.kindOf">
          <xsl:value-of select="substring-after(./mods:genre[@type='kindof']/@valueURI,'#')" />
      </field>
    </xsl:if>
    <xsl:for-each select="./mods:titleInfo">
      <xsl:if test="not(@transliteration) and not(@type)">
