...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  xmlns:mcrxsl="xalan://org.mycore.common.xml.MCRXMLFunctions" xmlns:mcrurn="xalan://org.mycore.urn.MCRXMLFunctions" exclude-result-prefixes="xlink mcr acl i18n mcrxsl websiteWriteProtection xalan mcrurn">
  ...
  <xsl:template name="printLatestObjects">
    ...
    <xsl:variable name="solrQuery" xmlns:encoder="xalan://java.net.URLEncoder">
      <xsl:value-of select="encoder:encode(concat('+objectType:', $objectType, ' +category:&quot;clausthal_status:published&quot;'))" />
    </xsl:variable>
    ...
    <xsl:choose>
      <xsl:when test="$overwriteLayout='true'">
        ...
        <xsl:call-template name="printLatestObjects.all">
          <xsl:with-param name="query2" select="$solrQuery" />
  ...
  <xsl:template name="printLatestObjects.all">
    ...
    <div id="latestmore">
      <a
        href="{$ServletsBaseURL}solr/select{$HttpSession}?q={$query2}&amp;fl=*,score&amp;sort={$sortBy}+desc&amp;rows=20&amp;mask=content/main/index.xml&amp;version=4.0">
